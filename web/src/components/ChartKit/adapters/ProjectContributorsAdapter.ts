...
export const ProjectContributorsAdapter: DatasetAdapter = {
  defaultType: 'bar',
  ...
  async fetcher(params: AdapterParams, signal?: AbortSignal): Promise<any> {
    ...
    if (!uuid) return []
    const url = `${base}/db/project_commits?select=author_login,committed_at&project_id=eq.${encodeURIComponent(String(uuid))}&order=committed_at.desc&limit=500`
